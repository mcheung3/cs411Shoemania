...
c.execute("""
	...
	WHERE shoes.id NOT IN (SELECT ratedlists.shoe_id FROM ratedlists WHERE ratedlists.user_id = %s)
	""", (user,))
notrated = c.fetchall()
...
shoe_data_test = np.array(notrated)
...
test_images = get_img_as_arr(shoe_data_test)
#end = time.time()
#print(end - start)
...
test_data = test_images.reshape((n_test, -1))
