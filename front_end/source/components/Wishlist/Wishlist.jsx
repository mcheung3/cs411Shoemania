...
class Wishlist extends Component {
	...
	componentDidMount(){
		...
		axios.get('http://ec2-18-188-247-213.us-east-2.compute.amazonaws.com:3000/wishlist/' + localStorage.getItem('username') ).then(response => {this.setState({data: response})});
	}
	...
	handleRemoveClick(event){
		...
		axios.delete('http://ec2-18-188-247-213.us-east-2.compute.amazonaws.com:3000/wishlist/', {params: {username: localStorage.getItem('username'), shoe_id: event.target.parentElement.parentElement.id}}, axiosConfig)
		.then((res) => {
			axios.get('http://ec2-18-188-247-213.us-east-2.compute.amazonaws.com:3000/wishlist/' + localStorage.getItem('username') ).then(response => {this.setState({data: response})});
		})
		.catch((err) => {
